# -*- coding: utf-8 -*-
"""
Created on Fri Jan 12 19:24:28 2024
...
spectral_centroids = librosa.feature.spectral_centroid(y=x[:80000], sr=sr)[0]#修改了位置参数(x[:80000], sr=sr)[0]
...
# t = np.linspace(t[0], t[-1], len(normalize(spectral_rolloff, axis=0)))
# plt.plot(t, normalize(spectral_rolloff), color='r')
plt.plot(t, normalize(spectral_rolloff[:len(t)]), color='r')#plt(x,y)这个语法x和y必须长度一致
...
y, sr = librosa.load(librosa.example('trumpet'))
 #语法更新 y, sr = librosa.load(librosa.util.example_audio_file())
hop_length = 512
...
plt.plot(x[:230], ac_global, '--', alpha=0.75, label='Global autocorrelation')
...
# plt.semilogx(freqs[1:], np.mean(tempogram[1:], axis=1)[:229],
#               label='Mean local autocorrelation', basex=2)
plt.semilogx(freqs[1:len(np.mean(tempogram[1:], axis=1)[:229]) + 1], np.mean(tempogram[1:], axis=1)[:229], label='Mean local autocorrelation', base=2)

# plt.semilogx(freqs[1:], ac_global[:229], '--', alpha=0.75,
              # label='Global autocorrelation', basex=2)
plt.semilogx(freqs[1:len(ac_global[:229]) + 1], ac_global[:229], '--', alpha=0.75, label='Global autocorrelation', base=2)
